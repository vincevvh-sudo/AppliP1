...
export default function EnseignantJeuNiveauPage() {
  ...
  const isEval = niveau?.type === "eval";
  const handleTermine = (reussi: boolean) => {
    alert(reussi ? "Bravo ! Exercice réussi." : "Tu peux réessayer.");
    // Après une évaluation, rester dans la partie Évaluations (choix Eval 1, 2, 3, 4). Sinon retour aux exercices du son.
    router.push(isEval ? `/enseignant/sons/evaluation/${sonId}` : `/enseignant/sons/jeu/${sonId}`);
  };
    ...
    <main className="relative min-h-screen overflow-hidden text-[#2d4a3e]">
      ...
      <header className="relative z-10 border-b border-[#2d4a3e]/10 bg-[#fef9f3]/95 backdrop-blur-md">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-5 py-4">
          <Link
            href={isEval ? `/enseignant/sons/evaluation/${sonId}` : `/enseignant/sons/jeu/${sonId}`}
