...
import {
  ADDITIONS20_SERIE_IDS_PARTAGEABLES,
  isAddition20SerieShared,
  setAddition20SerieShared,
  setAllAdditions20SeriesShared,
} from "../../../data/maths-partages";
import { TITRE_ADDITIONS_JUSQUE_20, getAdditions20Serie } from "../../../data/maths-additions-20";

export default function EnseignantMathsAdditions20Page() {
  const [partage, setPartage] = useState<Record<string, boolean>>({});
  const [hydrated, setHydrated] = useState(false);

  useEffect(() => {
    const next: Record<string, boolean> = {};
    for (const id of ADDITIONS20_SERIE_IDS_PARTAGEABLES) next[id] = isAddition20SerieShared(id);
    setPartage(next);
    setHydrated(true);
  }, []);
  ...
  const toggleSerie = useCallback((id: string) => {
    setPartage((p) => {
      const next = !p[id];
      setAddition20SerieShared(id, next);
      return { ...p, [id]: next };
    });
  }, []);

  const shareAll = useCallback(() => {
    setAllAdditions20SeriesShared(true);
    const next: Record<string, boolean> = {};
    for (const id of ADDITIONS20_SERIE_IDS_PARTAGEABLES) next[id] = true;
    setPartage(next);
  }, []);

  const shareNone = useCallback(() => {
    setAllAdditions20SeriesShared(false);
    const next: Record<string, boolean> = {};
    for (const id of ADDITIONS20_SERIE_IDS_PARTAGEABLES) next[id] = false;
    setPartage(next);
  }, []);
    ...
    <main className="relative min-h-screen overflow-hidden text-[#1f2933]">
      ...
      <header className="relative z-10 border-b border-[#1f2933]/10 bg-[#fef9ff]/95 backdrop-blur-md">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-5 py-4">
          <span className="font-display text-xl text-[#1f2933]">{TITRE_ADDITIONS_JUSQUE_20}</span>
          <Link
      ...
      <div className="relative z-10 mx-auto max-w-3xl px-5 py-10">
        ...
        <p className="mt-2 text-sm text-[#1f2933]/80">
          Chaque série contient 10 additions DU + U ou U + DU, avec un résultat toujours entre 10 et 20.
        </p>
        ...
        <div className="mt-6 space-y-3">
          {hydrated ? (
            ADDITIONS20_SERIE_IDS_PARTAGEABLES.map((id) => (
              <div key={id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[#1f2933]/15 bg-white/95 px-4 py-3 shadow-sm">
                <span className="font-medium text-[#1f2933]">Additions 20 - {id}</span>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <a href={`/enseignant/maths/additions-20/eleve/${id}`} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-[#4a7c5a] underline-offset-2 hover:underline">
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-2">
          {ADDITIONS20_SERIE_IDS_PARTAGEABLES.map((id) => {
            const { titre, questions } = getAdditions20Serie(id);
            return (
              <div key={`preview20-${id}`} className="rounded-xl border-2 border-[#1f2933]/15 bg-white/95 p-5 shadow">
