...
import {
  ADDITIONS_SOUSTRACTIONS20_SERIE_IDS_PARTAGEABLES,
  isAdditionSoustraction20SerieShared,
  setAdditionSoustraction20SerieShared,
  setAllAdditionsSoustractions20SeriesShared,
} from "../../../data/maths-partages";
import {
  TITRE_ADDITIONS_SOUSTRACTIONS_JUSQUE_20,
  getAdditionsSoustractions20Serie,
} from "../../../data/maths-additions-soustractions-20";

export default function EnseignantMathsAdditionsSoustractions20Page() {
  const [partage, setPartage] = useState<Record<string, boolean>>({});
  const [hydrated, setHydrated] = useState(false);

  useEffect(() => {
    const next: Record<string, boolean> = {};
    for (const id of ADDITIONS_SOUSTRACTIONS20_SERIE_IDS_PARTAGEABLES) next[id] = isAdditionSoustraction20SerieShared(id);
    setPartage(next);
    setHydrated(true);
  }, []);
  ...
  const toggleSerie = useCallback((id: string) => {
    setPartage((p) => {
      const next = !p[id];
      setAdditionSoustraction20SerieShared(id, next);
      return { ...p, [id]: next };
    });
  }, []);

  const shareAll = useCallback(() => {
    setAllAdditionsSoustractions20SeriesShared(true);
    const next: Record<string, boolean> = {};
    for (const id of ADDITIONS_SOUSTRACTIONS20_SERIE_IDS_PARTAGEABLES) next[id] = true;
    setPartage(next);
  }, []);

  const shareNone = useCallback(() => {
    setAllAdditionsSoustractions20SeriesShared(false);
    const next: Record<string, boolean> = {};
    for (const id of ADDITIONS_SOUSTRACTIONS20_SERIE_IDS_PARTAGEABLES) next[id] = false;
    setPartage(next);
  }, []);
    ...
    <main className="relative min-h-screen overflow-hidden text-[#1f2933]">
      ...
      <header className="relative z-10 border-b border-[#1f2933]/10 bg-[#fef9ff]/95 backdrop-blur-md">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-5 py-4">
          <span className="font-display text-xl text-[#1f2933]">{TITRE_ADDITIONS_SOUSTRACTIONS_JUSQUE_20}</span>
          <Link
      ...
      <div className="relative z-10 mx-auto max-w-3xl px-5 py-10">
        <h1 className="font-display text-2xl text-[#1f2933]">10 séries de 10 exercices</h1>
        <p className="mt-2 text-sm text-[#1f2933]/80">
          Mélange d&apos;additions (comme additions jusque 20) et de soustractions (départ entre 10 et 20).
        </p>
        ...
        <div className="mt-6 space-y-3">
          {hydrated ? (
            ADDITIONS_SOUSTRACTIONS20_SERIE_IDS_PARTAGEABLES.map((id) => (
              <div key={id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[#1f2933]/15 bg-white/95 px-4 py-3 shadow-sm">
                <span className="font-medium text-[#1f2933]">Add./Soust. 20 - {id}</span>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <a href={`/enseignant/maths/additions-soustractions-20/eleve/${id}`} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-[#4a7c5a] underline-offset-2 hover:underline">
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-2">
          {ADDITIONS_SOUSTRACTIONS20_SERIE_IDS_PARTAGEABLES.map((id) => {
            const { titre, questions } = getAdditionsSoustractions20Serie(id);
            return (
              <div key={`preview-mix-${id}`} className="rounded-xl border-2 border-[#1f2933]/15 bg-white/95 p-5 shadow">
