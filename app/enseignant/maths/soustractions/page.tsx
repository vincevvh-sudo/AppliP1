...
import {
  SOUSTRACTIONS_SERIE_IDS_PARTAGEABLES,
  isSoustractionSerieShared,
  setSoustractionSerieShared,
  setAllSoustractionsSeriesShared,
} from "../../../data/maths-partages";
import { TITRE_SOUSTRACTIONS_JUSQUE_10, getSoustractionSerie } from "../../../data/maths-soustractions";

export default function EnseignantMathsSoustractionsPage() {
  const [partage, setPartage] = useState<Record<string, boolean>>({});
  const [hydrated, setHydrated] = useState(false);

  useEffect(() => {
    const next: Record<string, boolean> = {};
    for (const id of SOUSTRACTIONS_SERIE_IDS_PARTAGEABLES) next[id] = isSoustractionSerieShared(id);
    setPartage(next);
    setHydrated(true);
  }, []);
  ...
  const toggleSerie = useCallback((id: string) => {
    setPartage((p) => {
      const next = !p[id];
      setSoustractionSerieShared(id, next);
      return { ...p, [id]: next };
    });
  }, []);

  const shareAll = useCallback(() => {
    setAllSoustractionsSeriesShared(true);
    const next: Record<string, boolean> = {};
    for (const id of SOUSTRACTIONS_SERIE_IDS_PARTAGEABLES) next[id] = true;
    setPartage(next);
  }, []);

  const shareNone = useCallback(() => {
    setAllSoustractionsSeriesShared(false);
    const next: Record<string, boolean> = {};
    for (const id of SOUSTRACTIONS_SERIE_IDS_PARTAGEABLES) next[id] = false;
    setPartage(next);
  }, []);
    ...
    <main className="relative min-h-screen overflow-hidden text-[#1f2933]">
      ...
      <header className="relative z-10 border-b border-[#1f2933]/10 bg-[#fef9ff]/95 backdrop-blur-md">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-5 py-4">
          <span className="font-display text-xl text-[#1f2933]">{TITRE_SOUSTRACTIONS_JUSQUE_10}</span>
          <Link
      ...
      <div className="relative z-10 mx-auto max-w-3xl px-5 py-10">
        ...
        <div className="mt-6 space-y-3">
          {hydrated ? (
            SOUSTRACTIONS_SERIE_IDS_PARTAGEABLES.map((id) => (
              <div key={id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[#1f2933]/15 bg-white/95 px-4 py-3 shadow-sm">
                <span className="font-medium text-[#1f2933]">Soustractions {id}</span>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <a href={`/enseignant/maths/soustractions/eleve/${id}`} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-[#4a7c5a] underline-offset-2 hover:underline">
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-2">
          {SOUSTRACTIONS_SERIE_IDS_PARTAGEABLES.map((id) => {
            const { titre, questions } = getSoustractionSerie(id);
            return (
              <div key={`preview-sub-${id}`} className="rounded-xl border-2 border-[#1f2933]/15 bg-white/95 p-5 shadow">
