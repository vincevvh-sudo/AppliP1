...
import {
  TITRE_SOUSTRACTIONS_JUSQUE_10,
  getSoustractionSerie,
  type SoustractionSerieId,
} from "../data/maths-soustractions";

type Props = {
  serieId: SoustractionSerieId;
  backHref: string;
  backLabel?: string;
};

export function MathsSoustractionsSeriePlay({ serieId, backHref, backLabel = "← Retour" }: Props) {
  const { titre, questions } = getSoustractionSerie(serieId);
    ...
    <main className="relative min-h-screen overflow-hidden text-[#1f2933]">
      ...
      <header className="relative z-10 border-b border-[#1f2933]/10 bg-[#fef9ff]/95 backdrop-blur-md">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-5 py-4">
          <span className="font-display text-xl text-[#1f2933]">
            {TITRE_SOUSTRACTIONS_JUSQUE_10} — {titre}
          </span>
          <Link
            href={backHref}
      ...
      <div className="relative z-10 mx-auto max-w-lg px-5 py-8">
        <p className="mb-6 text-sm text-[#1f2933]/80">
          10 soustractions dans la série. Chaque calcul commence à 10 ou moins.
        </p>
