...
type Props = {
  serieId: AdditionSoustraction20SerieId;
  backHref: string;
  backLabel?: string;
};

export function MathsAdditionsSoustractions20SeriePlay({ serieId, backHref, backLabel = "← Retour" }: Props) {
  const { titre, questions } = getAdditionsSoustractions20Serie(serieId);
    ...
    <main className="relative min-h-screen overflow-hidden text-[#1f2933]">
      ...
      <header className="relative z-10 border-b border-[#1f2933]/10 bg-[#fef9ff]/95 backdrop-blur-md">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-5 py-4">
          <span className="font-display text-xl text-[#1f2933]">
            {TITRE_ADDITIONS_SOUSTRACTIONS_JUSQUE_20} — {titre}
          </span>
          <Link href={backHref} className="rounded-full bg-[#1f2933]/10 px-4 py-2 text-sm font-medium text-[#1f2933] transition hover:bg-[#1f2933]/20">
            {backLabel}
          </Link>
        </div>
      </header>
      ...
      <div className="relative z-10 mx-auto max-w-lg px-5 py-8">
        <p className="mb-6 text-sm text-[#1f2933]/80">
          10 calculs mélangés (additions DU + U / U + DU et soustractions entre 10 et 20).
        </p>
        ...
        {!finished && current ? (
          <div className="rounded-2xl bg-white/95 p-6 shadow-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex justify-end">
                <button type="submit" disabled={input.trim() === ""} className="rounded-xl bg-[#4a7c5a] px-6 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">
                  Valider
                </button>
              </div>
            </form>
          ...
          <div className="rounded-2xl bg-white/95 p-6 shadow-lg">
              ...
              {questions.map((q, i) => {
                ...
                return (
                  <li key={q.id} className={correct ? "text-[#166534]" : "text-[#b91c1c]"}>
                    {q.a} {q.op} {q.b} = <span className="font-semibold">{hasVal ? val : "—"}</span>{" "}
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <Link href={backHref} className="rounded-xl bg-[#4a7c5a] px-5 py-2 text-sm font-semibold text-white">
