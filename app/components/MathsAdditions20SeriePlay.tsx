...
import {
  TITRE_ADDITIONS_JUSQUE_20,
  getAdditions20Serie,
  type Addition20SerieId,
} from "../data/maths-additions-20";

type Props = {
  serieId: Addition20SerieId;
  backHref: string;
  backLabel?: string;
};

export function MathsAdditions20SeriePlay({ serieId, backHref, backLabel = "← Retour" }: Props) {
  const { titre, questions } = getAdditions20Serie(serieId);
    ...
    <main className="relative min-h-screen overflow-hidden text-[#1f2933]">
      ...
      <header className="relative z-10 border-b border-[#1f2933]/10 bg-[#fef9ff]/95 backdrop-blur-md">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-5 py-4">
          <span className="font-display text-xl text-[#1f2933]">
            {TITRE_ADDITIONS_JUSQUE_20} — {titre}
          </span>
          <Link
            href={backHref}
      ...
      <div className="relative z-10 mx-auto max-w-lg px-5 py-8">
        <p className="mb-6 text-sm text-[#1f2933]/80">
          10 additions dans la série. Les opérations sont de type DU + U ou U + DU, avec une réponse entre 10 et 20.
        </p>
